...
- After Windows verification, restore locale, scaling, Explorer, registry startup entries, and power state. Remove test services, processes, directories, and Run/StartupApproved traces. Keep temporary scripts, screenshots, and verification results under `/tmp/commandtrayhost`; remove them after extracting the reportable result and before sending the final report. Do not create or retain evidence archives.
