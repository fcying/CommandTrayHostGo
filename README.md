...
| `skip_prerelease` | `true` | Ignore prereleases by default. Set to `false` to consider prereleases alongside formal releases. |

Only SemVer release versions such as `v1.2.3`, `v1.2.3-rc.1`, and `v1.2.3-dev.g<commit>` are accepted. The rolling GitHub `dev-latest` tag is also accepted when its prerelease title contains a SemVer version; assets are downloaded under `dev-latest` and verified against that title version. Each checker is bound to one validated `owner/repository` identity and returns one explicit outcome: unknown current version, up to date, or update available.
...
- Draft releases and unsupported tags are ignored. The rolling `dev-latest` prerelease is replaced on each prerelease publish, including same-repository pull-request publishes; other and fork pull requests run checks without publishing. `skip_prerelease=true` excludes prereleases; when set to `false`, formal releases and prereleases are considered together. If every valid candidate has a valid `published_at`, the most recently published release is selected, with SemVer breaking ties; if any timestamp is unavailable or invalid, the highest SemVer version is selected. A rolling `dev-latest` release with a newer base version is accepted, while a same-base development build updates whenever its version differs from the current prerelease. Older base versions are never installed.
- Prerelease publication enumerates all release pages before deleting existing prereleases and their tags, including the rolling `dev-latest` tag on a rerun. Formal releases are retained.
...
- Comparable development builds can use the rolling `dev-latest` prerelease when `skip_prerelease=false` and that release wins candidate selection. Non-comparable development builds do not check automatically. A manual check may report the latest release without claiming that the current version is comparable.
...
Privilege switching can be used while managed programs are running, without an application confirmation dialog. Elevating requests system UAC directly and retains a private non-elevated token of the original source user through the authenticated handoff. Dropping privileges uses this retained token; same-account direct administrator launches may use the normal desktop shell token instead. Cross-account launches without a retained original-user token fail safely rather than changing another user's token permissions. Temporary debug privilege is confined to a private thread token for cross-account process-handle access and is then removed. The new instance validates its requested privilege and configuration, receives the final runtime snapshot, and acknowledges readiness before the old host commits to shutdown. After final acknowledgement it waits for COMMIT or pipe disconnect rather than expiring an earlier handshake deadline. It then waits for old-host cleanup and restores runtime state even when caching is disabled. Job-owned programs restart; detached programs are not launched again. Root or active owned-entry `require_admin` requirements prevent dropping privilege. The original user token, SID and `-c` argument also survive online-update restart and rollback; retained tokens are never stored on disk.
