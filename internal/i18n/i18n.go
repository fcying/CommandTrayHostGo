...
func Text(language Language) Strings {
	if language == SimplifiedChinese {
		return Strings{
			...
			ReloadRequiresAdmin:    "重新加载的配置要求管理员权限. 请重启 CommandTrayHostGo 以应用该配置.",
			WatcherStopped:         "配置目录监控意外停止. 请重启 CommandTrayHostGo 以恢复热重载.",
			...
			NoUpdates:              "CommandTrayHostGo 已是最新版本.\n\n当前版本: %s\n最新版本: %s",
			UpdateAvailable:        "发现新版本 %s. 当前版本为 %s.\n\n将下载、验证签名并安装更新，然后自动重启。是否继续?",
	...
	return Strings{
		...
		ReloadRequiresAdmin:    "The reloaded config requires administrator privileges. Restart CommandTrayHostGo to apply it.",
		WatcherStopped:         "Configuration directory monitoring stopped unexpectedly. Restart CommandTrayHostGo to resume hot reload.",
		...
		NoUpdates:              "CommandTrayHostGo is up to date.\n\nCurrent version: %s\nLatest version: %s",
		UpdateAvailable:        "Version %s is available; the current version is %s.\n\nDownload, verify the signature, install the update, and restart automatically?",
