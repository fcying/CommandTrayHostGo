...
type ElevationState struct {
	...
	ReturnToken        windows.Token    `json:"-"`
}
...
type elevationEnvelope struct {
	...
	TargetElevated    bool
	SourceTokenHandle uint64 `json:",omitempty"`
	ReturnUserSID     string
}

// Close releases the original-user token retained by a received handoff.
func (s *ElevationState) Close() {
	if s != nil && s.ReturnToken != 0 {
		_ = s.ReturnToken.Close()
		s.ReturnToken = 0
	}
}
...
// A zero deadline waits until completion or peer disconnect, without a timeout.
...
func BeginElevation(target, configArgument, startupUserSID string, initial ElevationState, refresh func() (ElevationState, bool, error)) (result error) {
	...
	envelope := elevationEnvelope{SourcePID: uint32(os.Getpid()), Target: target, ConfigPath: configPath, State: initial, TargetElevated: targetElevated, ReturnUserSID: startupUserSID}
	if targetElevated {
		token, err := captureUnelevatedReturnToken(startupUserSID)
		if err != nil {
			return err
		}
		defer token.Close()
		envelope.SourceTokenHandle = uint64(token)
	}
	...
	if targetElevated {
		...
		receiver, err = launchUnelevated(target, parameters, filepath.Dir(target), startupUserSID, initial.ReturnToken)
	}
	if err != nil {
...
func openElevationSource(pipe windows.Handle, name, target string) (windows.Handle, uint32, error) {
	...
	if IsElevated() {
		access |= windows.PROCESS_DUP_HANDLE
	}
	source, err := windows.OpenProcess(access, false, actual)
	if IsElevated() && errors.Is(err, windows.ERROR_ACCESS_DENIED) {
		err = withDebugPrivilege(func() error {
			var openErr error
			source, openErr = windows.OpenProcess(access, false, actual)
			return openErr
		})
	}
	if err != nil {
...
func ReceiveElevation(name, target, configArgument, startupUserSID string) (*ElevationState, error) {
	...
	state, err := receiveElevationState(pipe, target, configArgument, pid, deadline, func(initial *elevationEnvelope) (windows.Token, error) {
		return receiveElevationReturnToken(source, initial, startupUserSID)
	})
	if err != nil {
		return nil, err
	}
...
// The source handle has already passed kernel PID and physical executable checks.
// Duplicate only the offered private token, never reopen or modify a source token.
func receiveElevationReturnToken(source windows.Handle, initial *elevationEnvelope, originalSID string) (windows.Token, error) {
	if originalSID == "" || initial.ReturnUserSID != originalSID {
		return 0, errors.New("elevation return user identity mismatch")
	}
	if !initial.TargetElevated {
		if initial.SourceTokenHandle != 0 {
			return 0, errors.New("unexpected return token for normal receiver")
		}
		return 0, requireUnelevatedUser(windows.GetCurrentProcessToken(), originalSID)
	}
	if initial.SourceTokenHandle == 0 {
		return 0, errors.New("missing original-user return token")
	}
	var duplicate windows.Handle
	if err := windows.DuplicateHandle(source, windows.Handle(initial.SourceTokenHandle), windows.CurrentProcess(), &duplicate, 0, false, windows.DUPLICATE_SAME_ACCESS); err != nil {
		return 0, fmt.Errorf("duplicate original-user return token: %w", err)
	}
	token := windows.Token(duplicate)
	if err := requireUnelevatedUser(token, originalSID); err != nil {
		token.Close()
		return 0, err
	}
	var session uint32
	...
	for _, check := range []struct{ class, want uint32 }{{windows.TokenType, windows.TokenPrimary}, {windows.TokenSessionId, session}} {
		var value, size uint32
		if err := windows.GetTokenInformation(token, check.class, (*byte)(unsafe.Pointer(&value)), uint32(unsafe.Sizeof(value)), &size); err != nil {
			token.Close()
			return 0, err
		}
		if size != uint32(unsafe.Sizeof(value)) || value != check.want {
			token.Close()
			return 0, errors.New("original-user return token type or session mismatch")
		}
	}
	return token, nil
}

func receiveElevationState(pipe windows.Handle, target, argument string, pid uint32, deadline time.Time, validateInitial func(*elevationEnvelope) (windows.Token, error)) (*ElevationState, error) {
	...
	returnToken, err := validateInitial(initial)
	defer func() {
		if returnToken != 0 {
			returnToken.Close()
		}
	...
	if final.TargetElevated != initial.TargetElevated || final.SourceTokenHandle != initial.SourceTokenHandle || final.ReturnUserSID != initial.ReturnUserSID || final.State.SourceConfigDigest != initial.State.SourceConfigDigest || len(final.State.Entries) != len(initial.State.Entries) {
	...
	// Once prepared, the source may have committed and exited while we were
	// descheduled. Consume its buffered COMMIT regardless of the old deadline;
	// an aborted source closes the pipe instead and cannot authorize activation.
	if _, err := readElevationFrame(pipe, elevationCommit, time.Time{}); err != nil {
		return nil, err
	}
	final.State.ReturnToken = returnToken
	returnToken = 0
